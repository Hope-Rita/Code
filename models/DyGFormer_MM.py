...
class DyGFormer_MM(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        """
        super(DyGFormer_MM, self).__init__()
        ...
        # ])
        self.transformers = nn.ModuleList([
            MonarchMixerSequenceMixing(d_model=self.num_channels * self.channel_embedding_dim, l_max=self.max_input_sequence_length, dropout=self.dropout)
            for _ in range(self.num_layers)
        ])

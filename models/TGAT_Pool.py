...
class TGAT_Pool(nn.Module):

    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, pool_kernel_size: int, num_layers: int = 2, num_heads: int = 2, dropout: float = 0.1, device: str = 'cpu'):
        """
        TGAT model.
        ...
        self.attention_dim = self.node_feat_dim + self.edge_feat_dim + self.time_feat_dim
        ...
        self.temporal_conv_layers = nn.ModuleList([TransformerEncoder(attention_dim = self.attention_dim,
                                                                      num_heads=self.num_heads,
                                                                      pool_kernel_size=pool_kernel_size,
                                                                      dropout=self.dropout) for _ in range(num_layers)])
        # self.temporal_conv_layers_pool = nn.ModuleList([MultiPooling(kernel_size=kernel_size) for _ in range(num_layers)])

        # follow the TGAT paper, use merge layer to combine the attention results and node original feature
        self.merge_layers = nn.ModuleList([MergeLayer(input_dim1=self.attention_dim, input_dim2=self.node_feat_dim,
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if current_layer_num == 0:
            ...
            # Tensor, output shape (batch_size, node_feat_dim + time_feat_dim + edge_feat_dim)

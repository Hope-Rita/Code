...
class GraphMixer_Pool(nn.Module):

    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, num_tokens: int, num_layers: int = 2, channel_kernel_size: int or list = 1,
                 channel_dim_expansion_factor: float = 4.0, token_kernel_size: int or list = 1, dropout: float = 0.1, device: str = 'cpu'):
        """
        TCL model.
        ...
        """
        super(GraphMixer_Pool, self).__init__()
        ...
        token_lengths = [token_kernel_size]
        # for i in range(self.num_layers):
        #     current_lengths = token_lengths[i]
        #     token_lengths.append(current_lengths)
        self.mlp_mixers = nn.ModuleList([
            MLPMixer(num_tokens=self.num_tokens // self.patch_size, num_channels=self.num_channels, dropout=self.dropout,
        ...
        self.patches_layer = nn.Linear(in_features=self.patch_size * self.num_channels, out_features=self.num_channels, bias=True)
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        combined_features = self.get_patches(combined_features, patch_size=self.patch_size)
        combined_features = self.patches_layer(combined_features)
    ...
    def get_patches(self, padded_features: torch.Tensor = None, patch_size: int = 1):
        """
        get the sequence of patches for nodes
        :param padded_features: Tensor, shape (batch_size, max_seq_length, node_feat_dim)
        :param patch_size: int, patch size
        :return:
        """
        assert padded_features.shape[1] % patch_size == 0
        num_patches = padded_features.shape[1] // patch_size
        feat_dim = padded_features.shape[-1]
        ...
        for patch_id in range(num_patches):
            ...
            patches_features.append(
                padded_features[:, start_idx: end_idx, :])

        batch_size = len(padded_features)
        # Tensor, shape (batch_size, num_patches, patch_size * node_feat_dim)
        patches_nodes_neighbor_node_raw_features = torch.stack(patches_features, dim=1).reshape(
...
class MLPMixer(nn.Module):
    ...
    def __init__(self, num_tokens: int, num_channels: int, token_kernel_size: int,
                 channel_kernel_size: int, dropout: float, channel_dim_expansion_factor: float = 4.0):
        """
        MLP Mixer.
        :param num_tokens: int, number of tokens

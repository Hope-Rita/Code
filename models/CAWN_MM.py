...
from models.modules import TransformerEncoder_Original as TransformerEncoder
from utils.utils import NeighborSampler
from models.mm.monarch_mixer_sequence_mixer import MonarchMixerSequenceMixing



class CAWN_MM(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler, num_neighbors: int,
        ...
        self.walk_encoder = WalkEncoder(input_dim=self.node_feat_dim + self.edge_feat_dim + self.time_feat_dim + self.position_feat_dim, num_neighbors = num_neighbors,
...
class WalkEncoder(nn.Module):
    ...
    def __init__(self, input_dim: int, position_feat_dim: int, output_dim: int, num_walk_heads: int, num_neighbors: int, dropout: float = 0.1):
        """
        ...
        self.transformer_encoder = MonarchMixerSequenceMixing(d_model=self.attention_dim, l_max=num_neighbors, dropout=self.dropout)
    ...
    def forward(self, neighbor_raw_features: torch.Tensor, neighbor_time_features: torch.Tensor, edge_features: torch.Tensor,
        ...
        combined_features = self.transformer_encoder(u=combined_features).mean(dim=-2)
        # Tensor, shape (batch_size, self.output_dim)

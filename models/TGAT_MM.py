...
from models.mm.monarch_mixer_sequence_mixer import MonarchMixerSequenceMixing


class TGAT_MM(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler, num_neighbors: int,
        ...
        self.temporal_conv_layers = nn.ModuleList([MonarchMixerSequenceMixing(d_model=self.attention_dim, l_max=num_neighbors, dropout=self.dropout) for _ in range(num_layers)])
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if current_layer_num == 0:
            ...
            output = self.temporal_conv_layers[current_layer_num - 1](u=combined_features).mean(dim=1)

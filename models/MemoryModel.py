...
from models.modules import TimeEncoder, MergeLayer, MultiHeadAttention, TransformerEncoder
...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler, pool_kernel_size:int,
        ...
        if self.model_name in ['TGN', 'TGN_Pool']:
        ...
        elif self.model_name in ['DyRep', 'JODIE', 'DyRep_Pool']:
        ...
        if self.model_name == 'JODIE':
            ...
            self.embedding_module = GraphAttentionEmbedding_Pool(node_raw_features=self.node_raw_features,
                                                            edge_raw_features=self.edge_raw_features,
                                                            neighbor_sampler=neighbor_sampler,
                                                            time_encoder=self.time_encoder,
                                                            node_feat_dim=self.node_feat_dim,
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        elif self.model_name in ['TGN', 'DyRep', 'DyRep_Pool', 'TGN_Pool']:
        ...
        if self.model_name in ['DyRep', 'DyRep_Pool']:
    ...
    def set_neighbor_sampler(self, neighbor_sampler: NeighborSampler):
        ...
        :return:
        """
        assert self.model_name in ['TGN', 'DyRep', 'DyRep_Pool', 'TGN_Pool'], f'Neighbor sampler is not defined in model {self.model_name}!'
...
class MemoryBank(nn.Module):
    ...
    def get_node_last_updated_times(self, unique_node_ids: np.ndarray):
        ...
        :return:
        """
        return self.node_last_updated_times[torch.from_numpy(unique_node_ids).type(torch.long)]
...
class MemoryUpdater(nn.Module):
    ...
    def get_updated_memories(self, unique_node_ids: np.ndarray, unique_node_messages: torch.Tensor,
        ...
        updated_node_memories[torch.from_numpy(unique_node_ids).type(torch.long)] = self.memory_updater(unique_node_messages,
                                                                                       updated_node_memories[torch.from_numpy(unique_node_ids).type(torch.long)])

        # Tensor, shape (num_nodes, )
        updated_node_last_updated_times = self.memory_bank.node_last_updated_times.data.clone()
        updated_node_last_updated_times[torch.from_numpy(unique_node_ids).type(torch.long)] = torch.from_numpy(unique_node_timestamps).type(torch.long).float().to(unique_node_messages.device)
...
class GraphAttentionEmbedding_Pool(nn.Module):
    ...
    def __init__(self, node_raw_features: torch.Tensor, edge_raw_features: torch.Tensor, neighbor_sampler: NeighborSampler,
        ...
        :param dropout: float, dropout rate
        """
        super(GraphAttentionEmbedding_Pool, self).__init__()
        ...
        self.temporal_conv_layers = nn.ModuleList(
            [TransformerEncoder(attention_dim=self.node_feat_dim + self.edge_feat_dim + self.time_feat_dim,
                                pool_kernel_size=self.pool_kernel_size,
                                num_heads=self.num_heads,
                                dropout=self.dropout) for _ in range(num_layers)])
        # follow the TGN paper, use merge layer to combine 1) the attention results, and 2) node raw feature + node memory
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if current_layer_num == 0:
            ...
            combined_features = torch.cat([neighbor_node_conv_features, neighbor_edge_features, neighbor_time_features], dim = 2)

            # temporal graph convolution
            # Tensor, output shape (batch_size, node_feat_dim + time_feat_dim)
            output = self.temporal_conv_layers[current_layer_num - 1](inputs = combined_features).mean(dim = 1)

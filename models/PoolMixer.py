...
class PoolMixer(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 channel_kernel_size: int = 1, token_kernel_size: int = 1,
        ...
        self.projection_layer = nn.ModuleDict({
            ...
            'combined': nn.Linear(in_features=self.num_channels * self.channel_embedding_dim, out_features=self.num_channels * self.channel_embedding_dim, bias=True),
        ...
        #     TransformerEncoder(attention_dim=self.num_channels * self.channel_embedding_dim, num_heads=self.num_heads, pool_kernel_size= self.pool_kernel_size, dropout=self.dropout)
        #     for _ in range(self.num_layers)
        # ])
        self.axis_mixers = nn.ModuleList([
            AxisMixer_1( num_channels=self.num_channels * self.channel_embedding_dim, dropout = self.dropout,
                     channel_kernel_size=channel_kernel_size, token_kernel_size=token_kernel_size)
            for _ in range(self.num_layers)
        ])
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        src_nums = src_padded_nodes_neighbor_node_raw_features.shape[1]
        dst_nums = dst_padded_nodes_neighbor_node_raw_features.shape[1]

        # Tensor, shape (batch_size, src_num + dst_num, channel_embedding_dim)
        ...
        combined_data = [padded_nodes_neighbor_node_raw_features, padded_nodes_edge_raw_features,
        ...
        combined_data = torch.stack(combined_data, dim=2)
        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim)
        combined_data = combined_data.reshape(batch_size, src_nums + dst_nums, self.num_channels * self.channel_embedding_dim)
        combined_data = self.projection_layer['combined'](combined_data)

        # Tensor, shape (batch_size, src_num_patches + dst_num_patches, num_channels * channel_embedding_dim)
        for axis_mixer in self.axis_mixers:
            combined_data = axis_mixer(combined_data)

        # src_patches_data, Tensor, shape (batch_size, src_num_patches, num_channels * channel_embedding_dim)
        src_patches_data = combined_data[:, : src_nums, :]
        # dst_patches_data, Tensor, shape (batch_size, dst_num_patches, num_channels * channel_embedding_dim)
        dst_patches_data = combined_data[:, src_nums: src_nums + dst_nums, :]
...
class AxisMixer(nn.Module):
    ...
    def __init__(self,  num_channels: int, token_kernel_size: int,
                 channel_kernel_size: int, dropout: float):
        """
        MLP Mixer.
        ...
        :param dropout: float, dropout rate
        """
        super(AxisMixer, self).__init__()

        # self.token_norm = nn.LayerNorm(num_tokens)
        self.token_feedforward = PoolNet(kernel_size=token_kernel_size, dropout = dropout)

        self.channel_norm = nn.LayerNorm(num_channels)
        self.channel_feedforward = PoolNet(kernel_size=channel_kernel_size, dropout = dropout)
    ...
    def forward(self, input_tensor: torch.Tensor):
        ...
        """
        # mix tokens
        normalized_shape = tuple((input_tensor.shape[1],))
        # Tensor, shape (batch_size, num_channels, num_tokens)
        tokens_tensor = F.layer_norm(input_tensor.permute(0, 2, 1),normalized_shape)
        # Tensor, shape (batch_size, num_tokens, num_channels)
        tokens_tensor = self.token_feedforward(tokens_tensor).permute(0, 2, 1)
        # Tensor, shape (batch_size, num_tokens, num_channels), residual connection
        tokens_tensor = tokens_tensor + input_tensor

        # mix channels
        # Tensor, shape (batch_size, num_tokens, num_channels)
        channel_tensor = self.channel_norm(input_tensor)
        # Tensor, shape (batch_size, num_tokens, num_channels)
        channel_tensor = self.channel_feedforward(channel_tensor)
        # Tensor, shape (batch_size, num_tokens, num_channels), residual connection
        channel_tensor = channel_tensor + input_tensor

        return tokens_tensor + channel_tensor


class AxisMixer_1(nn.Module):
    ...
    def __init__(self, num_channels: int, token_kernel_size: int,
        ...
        :param dropout: float, dropout rate
        """
        super(AxisMixer_1, self).__init__()
    ...
    def forward(self, input_tensor: torch.Tensor):
        ...
        tokens_tensor = self.token_feedforward(tokens_tensor).permute(0, 2, 1)
        # Tensor, shape (batch_size, num_tokens, num_channels), residual connection
        output_tensor = tokens_tensor + input_tensor

        # mix channels
        # Tensor, shape (batch_size, num_tokens, num_channels)
        channel_tensor = self.channel_norm(output_tensor)
        # Tensor, shape (batch_size, num_tokens, num_channels)
        channel_tensor = self.channel_feedforward(channel_tensor)
        # Tensor, shape (batch_size, num_tokens, num_channels), residual connection
        output_tensor = channel_tensor + output_tensor
